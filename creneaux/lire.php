...
// if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    // include_once '../models/Creneaux.php';
    ...
    // $creneau = new Creneaux($db);
    $rdv = new Reservations($db);

    // On récupère les données
    $data = json_decode(file_get_contents("php://input"));
    // echo'hiiiiiii';
    $rdv->date_reservation = $data->date_reservation;
    
    $s = $rdv->lireCreneauxDispo();
    ...
    if($s->rowCount() > 0){
        ...
        while($row = $s->fetch(PDO::FETCH_ASSOC)){
            ...
            $tableauReservations['creneaux'][] = $prod;
        }
